...
def main():
    ...
    forward_button['command']= lambda: mqtt_client.send_message("say_it", ["forward",right_speed_entry.get(),left_speed_entry.get()])
    root.bind('<Up>', lambda event: print("Forward key"))

    left_button = ttk.Button(main_frame, text="Left")
    left_button.grid(row=3,column=0)
    forward_button['command'] = lambda: mqtt_client.send_message("say_it", ["Left", right_speed_entry.get(),
    ...
    forward_button['command'] = lambda: mqtt_client.send_message("say_it", ["Right",right_speed_entry.get(),
                                                                            left_speed_entry.get()])
    root.bind('<Right>', lambda event: print("Right key"))

    back_button = ttk.Button(main_frame, text="Back")
    back_button.grid(row=4,column=1)
    forward_button['command'] = lambda: mqtt_client.send_message("say_it", ["Back", right_speed_entry.get(),
                                                                            left_speed_entry.get()])
